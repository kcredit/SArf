...
compare_spatial_models <- function(formula, data, spatial_weights, 
  ...
  if (include_naive_rf) {
    ...
    if (!is.null(naive_rf)) {
      ...
      results$Naive_RF <- c(naive_metrics,
                           Morans_I = naive_moran["statistic"],
                           Morans_pval = naive_moran["p_value"])
  ...
  if ("OLS" %in% compare_models) {
    ...
    results$OLS <- c(ols_metrics, 
                     Morans_I = ols_moran["statistic"],
                     Morans_pval = ols_moran["p_value"])
  ...
  if ("SAR" %in% compare_models) {
    ...
    if (!is.null(sar_model)) {
      ...
      results$SAR <- c(sar_metrics,
                      Morans_I = sar_moran["statistic"],
                      Morans_pval = sar_moran["p_value"])
  ...
  if ("SEM" %in% compare_models) {
    ...
    if (!is.null(sem_model)) {
      ...
      results$SEM <- c(sem_metrics,
                      Morans_I = sem_moran["statistic"],
                      Morans_pval = sem_moran["p_value"])
  ...
  if ("SAC" %in% compare_models) {
    ...
    if (!is.null(sac_model)) {
      ...
      results$SAC <- c(sac_metrics,
                      Morans_I = sac_moran["statistic"],
                      Morans_pval = sac_moran["p_value"])
  ...
  results$RF_Spatial_CV <- c(
    RMSE = rf_rmse,
    R2 = rf_r2,
    Morans_I = rf_moran["statistic"],
    Morans_pval = rf_moran["p_value"]
  )
